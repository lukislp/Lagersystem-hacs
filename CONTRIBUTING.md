# Contributing to the LagerSystem Home Assistant integration
...
- **Conventional Commits.** The version and the changelog are generated from the commit messages
  ...
  The release also syncs the version into `custom_components/lagersystem/manifest.json`, which is
...
`hacs-ci.yml`, called with `component: lagersystem` and `fuzz-script: fuzz/fuzz_sensors.py`. That
...
pytest tests/ -v --cov=custom_components/lagersystem --cov-report=json:coverage.json
```
...
PYTHONPATH=. python fuzz/fuzz_sensors.py -max_total_time=30 -rss_limit_mb=1024
```

To try the integration against a real Home Assistant, copy `custom_components/lagersystem` into your
